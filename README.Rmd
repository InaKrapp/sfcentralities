...
For certain networks, a high closeness value might be calculated for points which are at the periphery of a network. This can happen because while points which are only reachable 
points which are not reachable at all have a 'NA' distance value and therefore do not enter the closeness calculation. 
To avoid this issue, points which can not be reached from a large number of other points in the network can be discharged from the calculation. 
```{r}
# Use the hampi streetnet as example:
